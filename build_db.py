...
CREATE TABLE IF NOT EXISTS {jsonl_file_path.split('_')[1].split('.')[0]} (
  ...
  for line in s1:
    ...
    cur.execute(f'''
      INSERT OR IGNORE INTO {jsonl_file_path.split('_')[1].split('.')[0]} (id, season, name, type, motto, category, region, flag, cardcategory, population, description, badges, trophies)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ''', (json_data['ID'], json_data['SEASON'], json_data['NAME'], json_data['TYPE'], json_data['MOTTO'], json_data['CATEGORY'], json_data['REGION'], json_data['FLAG'], json_data['CARDCATEGORY'], popsa, json_data['DESCRIPTION'], json.dumps(json_data['BADGES']), json.dumps(json_data['TROPHIES'])))
