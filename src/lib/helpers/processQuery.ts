...
export async function loadCards(url: URL) {
	if (building) return;
	...
	if (!ua) return { err: "No UA provided" };
	...
	try {
		let { cards: data, total, limit: fetchedLimit, page: fetchedPage } = await fetchCards(fullQ);
		...
		return {
			cards,
			total: checkingClient ? cards.length : total,
			limit: fetchedLimit,
			page: fetchedPage,
			err,
			ua,
