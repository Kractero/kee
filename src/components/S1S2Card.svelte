...
>
	<div class={`deckcard-container border-2 border-solid ${card.cte === true && 'opacity-50'}`}>
		<div
			class={`deckcard deckcard-season-${card.season}`}
			data-cardid={card.id}
		...
		>
			<figure class={`front deckcard-category-${card.cardcategory}`}>
				<div
					class="deckcard-flag"
					style={`background-image: ${card.flag.includes('./') ? `url(${card.flag.replace('./', '')})` : `url(https://www.nationstates.net/images/cards/s${card.season}/${card.flag})`};`}
				></div>
